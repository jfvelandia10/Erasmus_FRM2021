#fan function from package fanplot modified by Juan Velandia 12-05-2021
#Input directly a value for x values
#Instead of median calculates a mean value (same input)
fanJF<-function (data = NULL, data.type = "simulations", style = "fan", TR=NULL,
          ...
          ln = if (length(probs) < 10) probs else probs[round(probs, 
                                                              2) %in% round(seq(0.1, 0.9, 0.1), 2)], ln.col = if (style == 
                                                                                                                  "spaghetti") "gray" else NULL, med.ln = if (type == 
                                                                                                                                                              "interval") TRUE else FALSE, med.col = "orange", rlab = ln, 
          rpos = 4, roffset = 0.1, rcex = 0.8, rcol = NULL, llab = FALSE, 
          lpos = 2, loffset = roffset, lcex = rcex, lcol = rcol, upplab = "U", 
          lowlab = "L", medlab = if (type == "interval") "M" else NULL, 
      ...
      if (style == "boxfan") {
        for (i in 1:nrow(pp)) {
          for (j in match(ln0, p)) {
            lines(x = tt[i] + c(-0.5, 0.5) * space, 
                  y = rep(pp[i, j], 2), col = ln.col)
          }
        }
      }
